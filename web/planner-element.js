import { plannerTranslator, plannerError, plannerLabel } from "./planner-catalogs.js";
    ...
    class PlannerElement extends HTMLElement {
        ...
        async #connect() {
            ...
            if (contribution === undefined) {
                this.#unavailable(this.#t("The host did not provide a planner generation."));
                return;
            }
            if (contribution.edits === undefined) {
                this.#unavailable(this.#t("Update the host to protect unsaved planner edits before using this package."));
                return;
            }
            const runtime = runtimeFor(contribution.addon.generation);
            if (runtime === undefined || runtime.signal.aborted) {
                this.#unavailable(this.#t("This DM Tools generation is no longer active."));
                return;
            }
            try {
        ...
        async #reload(loading, live = false) {
            ...
            try {
                ...
                if (!request.signal.aborted && this.#runtime === runtime) {
                    ...
                    this.#fail(error, this.#t("Could not load planning data."));
                }
            }
            finally {
                if (this.#runtime === runtime && this.#readRequest === request && !request.signal.aborted && this.isConnected) {
        ...
        #applyTarget() {
            ...
            try {
                ...
                this.#scopeId = null;
                this.#selectedId = undefined;
                this.#fail(error, this.#t("This planning item no longer exists."));
            }
        }
        #invalidLink(error) {
            if (this.#snapshot && this.#runtime) {
                this.#invalidTarget = true;
                this.#fail(error, this.#t("Invalid planner link."));
                this.#render();
                return;
            }
            this.#readRequest?.abort();
        ...
        #render(captureViewport = true) {
            ...
            if (snapshot === undefined) {
                ...
                if (!this.#busy)
                    this.append(actionButton(this.ownerDocument, this.#t("Reload planner"), () => void this.#reload(this.#t("Loading story planner…"))));
                return;
            }
            ...
            title.textContent = this.#t("Story Planner");
            const subtitle = document.createElement("p");
            subtitle.textContent = this.#t("An editable tree of local story-flow canvases.");
            ...
            const refresh = actionButton(document, this.#t("Reload planner"), () => void this.#reload(this.#t("Reloading planning data…")));
        ...
        #breadcrumbs(document, snapshot) {
            ...
            nav.setAttribute("aria-label", this.#t("Planner scope"));
        ...
        #atlas(document) {
            ...
            title.textContent = this.#t("Atlas");
            aside.append(title);
            for (const [kind, label] of [["plotline", this.#t("Plotline")], ["quest", this.#t("Quest")], ["event", this.#t("Event")], ["branch", this.#t("Branch")]])
                aside.append(actionButton(document, `+ ${label}`, () => void this.#create(kind), kind));
            if (this.#newItem)
                aside.append(actionButton(document, this.#t("Resume new item"), () => this.#create(this.#newItem.kind)));
            const help = document.createElement("p");
            help.textContent = this.#t("Click to select; Shift-click adds to the selection. Drag empty canvas to select a group. Double-click or Enter to edit. Shift+Enter opens a plotline or quest. Middle-drag or Alt-drag pans.");
            aside.append(help);
            return aside;
        }
        #canvas(document, snapshot) {
            ...
            svg.setAttribute("aria-label", this.#t("Story flow"));
            ...
            for (const flow of localFlows(snapshot, this.#scopeId)) {
                ...
                path.setAttribute("aria-label", flow.label || this.#t("{0} to {1}", { "0": flow.sourceId, "1": flow.targetId }));
            ...
            for (const item of children) {
                ...
                kind.className = "kind";
                kind.textContent = plannerLabel(subtype(item), this.#t);
                const title = document.createElement("h3");
                title.textContent = item.title;
                const summary = document.createElement("p");
                summary.textContent = item.summary || this.#t("Needs details");
                ...
                port.setAttribute("aria-label", this.#t("Connect from {0}", { "0": item.title }));
                port.title = this.#t("Click or drag to connect to another card");
                card.append(kind, title, summary, port);
                stage.append(card);
            }
            if (children.length === 0) {
                const empty = messageBlock(document, this.#t("This canvas is empty. Add a planning item from the Atlas."), "status");
            ...
            const expand = actionButton(document, this.#fullscreen ? labels.exit : labels.fullscreen, () => { if (!this.#busy)
                void (this.#fullscreen ? document.exitFullscreen() : this.requestFullscreen()).catch(error => this.#invalid(plannerError(error, this.#t, this.#t("Fullscreen is unavailable.")))); });
            ...
            toolbar.append(help);
            const reset = actionButton(document, this.#t("Reset layout"), () => void this.#resetLayout());
            toolbar.append(reset);
            if (this.#undoDelete)
                toolbar.append(actionButton(document, this.#t("Undo last deletion"), () => void this.#undoDeletion()));
        ...
        #selectionToolbar(document, snapshot) {
            ...
            toolbar.setAttribute("role", "group");
            toolbar.setAttribute("aria-label", this.#t("Selection actions"));
            const count = this.#selection.items.size + this.#selection.flows.size, label = document.createElement("span");
            label.textContent = count ? this.#t("{0} selected", { "0": count }) : this.#t("Select a card or flow");
            ...
            if (item) {
                toolbar.append(actionButton(document, this.#t("Edit selected"), () => this.#edit(item.id)));
                if (item.kind === "plotline" || item.kind === "quest")
                    toolbar.append(actionButton(document, this.#t("Enter this canvas"), () => this.#openCanvas(item.id)));
            }
            if (!this.#selection.items.size && this.#selection.flows.size === 1) {
                ...
                if (flow)
                    toolbar.append(actionButton(document, this.#t("Edit selected flow"), () => {
            ...
            if (this.#selection.items.size === 2 && !this.#selection.flows.size)
                toolbar.append(actionButton(document, this.#t("Connect selected"), () => {
                    const [source, target] = [...this.#selection.items];
                    if (!source || !target)
                        return;
                    this.#edit(source, "links");
                    const select = this.querySelector('form[data-create-flow] select[name="targetId"]');
                    if (select) {
                        select.value = target;
            ...
            if (count)
                toolbar.append(actionButton(document, this.#t("Delete selection"), () => void this.#deleteSelection(), "danger"));
            return toolbar;
        }
        #editor(document, snapshot) {
            const dialog = document.createElement("dialog");
            dialog.className = "dm-planner-dialog dm-planner-inspector";
            dialog.setAttribute("aria-label", this.#t("Edit planning item"));
            ...
            const closeEditor = () => { if (isNew)
                ...
                this.#closeEditor(); };
            const close = actionButton(document, isNew ? this.#t("Cancel creation") : this.#t("Close editor"), closeEditor);
            close.dataset["viewAction"] = "";
            const reload = actionButton(document, this.#t("Reload planner"), () => void this.#reload());
            reload.dataset["viewAction"] = "";
            const saveItem = actionButton(document, this.#t("Save item"), () => undefined, "primary");
            ...
            if (this.#needsReload)
                body.append(messageBlock(document, this.#t("Reload the planner before making another change. Your edits are retained; review the saved data before retrying."), "alert"));
            ...
            for (const [id, label] of [["details", this.#t("Details")], ["links", this.#t("Links")], ["notes", this.#t("Notes")]]) {
            ...
            refreshStructure();
            const save = actionButton(document, this.#t("Save details"), () => undefined, "primary");
            save.type = "submit";
            form.append(save);
            aside.append(form);
            if (isNew)
                aside.prepend(messageBlock(document, this.#t("This item is not saved yet. Save it before adding links or notes; Cancel creation leaves the campaign unchanged."), "status"));
            else {
                if (selected.kind === "plotline" || selected.kind === "quest")
                    aside.append(actionButton(document, this.#t("Enter this canvas"), () => this.#openCanvas(selected.id)));
        ...
        #flowEditor(document, snapshot, selected) {
            ...
            form.append(select, selectField(document, this.#t("Flow type"), "kind", selected.kind === "branch" ? "option" : "continues", flowKindOptions(selected, this.#t)), textField(document, this.#t("Flow label"), "label", ""));
            ...
            for (const flow of attached) {
                ...
                description.textContent = `${flow.sourceId === selected.id ? this.#t("To") : this.#t("From")} ${peer?.title ?? peerId}${flow.label === "" ? "" : `: ${flow.label}`}`;
                const entry = document.createElement("article");
                entry.className = "dm-planner-flow-entry";
                entry.dataset["flowId"] = flow.id;
                row.append(description, actionButton(document, this.#t("Remove flow"), () => void this.#deleteFlow(flow, selected.id), "danger"));
                entry.append(row);
                const editor = document.createElement("details");
                const editLabel = document.createElement("summary");
                editLabel.textContent = this.#t("Edit flow");
                editor.append(editLabel);
                editor.open = this.#drafts.has(`planning_flow_links:${flow.id}`);
                const editForm = document.createElement("form");
                editForm.setAttribute("aria-label", this.#t("Edit story flow"));
                const source = snapshot.items.find(item => item.id === flow.sourceId);
                editForm.append(selectField(document, this.#t("Flow type"), "kind", flow.kind, flowKindOptions(source, this.#t)), textField(document, this.#t("Flow label"), "label", flow.label));
        ...
        #annotations(document, snapshot, selected) {
            const section = document.createElement("section");
            const title = document.createElement("h3");
            title.textContent = this.#t("Planning annotations");
            ...
            createLabel.textContent = this.#t("Add reference");
            create.append(createLabel);
            create.open = this.#drafts.has(`new-reference:${selected.id}`);
            const createForm = document.createElement("form");
            createForm.setAttribute("aria-label", this.#t("Create reference"));
            ...
            for (const reference of snapshot.references.filter((entry) => entry.itemId === selected.id)) {
                ...
                const quantity = textField(document, this.#t("Quantity"), "quantity", String(reference.quantity));
                ...
                save.type = "submit";
                form.append(save, actionButton(document, this.#t("Delete reference"), () => void this.#deleteRecord("planning_references", reference.id, this.#t("Reference deleted."), selected.id), "danger"));
                section.append(form);
            ...
            const anchorOptions = [[`item:${selected.id}`, this.#t("Whole item")], ...attachedFlows.map(flow => [`flow:${flow.id}`, flowDescription(snapshot, flow)])];
            for (const consequence of snapshot.consequences.filter(entry => entry.anchor["scope"] === "item" ? entry.anchor["itemId"] === selected.id : attachedFlows.some(flow => flow.id === entry.anchor["flowId"]))) {
                ...
                form.append(textField(document, this.#t("Consequence"), "title", consequence.title), selectField(document, this.#t("Kind"), "kind", consequence.kind, [["world", this.#t("World")], ["reward", this.#t("Reward")], ["information", this.#t("Information")], ["complication", this.#t("Complication")]]), textArea(document, this.#t("Details"), "body", consequence.body, 3));
                const save = actionButton(document, this.#t("Save consequence"), () => undefined, "primary");
                save.type = "submit";
                form.append(save, actionButton(document, this.#t("Delete consequence"), () => void this.#deleteRecord("planning_consequences", consequence.id, this.#t("Consequence deleted."), selected.id), "danger"));
                section.append(form);
        ...
        #notes(document, snapshot, selected) {
            ...
            for (const note of snapshot.notes.filter((entry) => entry.anchorIds.length === 0 || entry.anchorIds.includes(selected.id))) {
                ...
                form.append(textField(document, this.#t("DM note"), "title", note.title), textArea(document, this.#t("Private details"), "body", note.body, 4));
                const save = actionButton(document, this.#t("Save note"), () => undefined, "primary");
                save.type = "submit";
                form.append(save, actionButton(document, this.#t("Delete note"), () => void this.#deleteRecord("dm_notes", note.id, this.#t("DM note deleted."), selected.id), "danger"));
                section.append(form);
                form.dataset["noteId"] = note.id;
                if (note.anchorIds.length === 0)
                    form.prepend(messageBlock(document, this.#t("Unanchored note — link it to a planning item below."), "status"));
        ...
        async #saveItem(item, form) {
            ...
            if (next.title === "") {
                this.#message = this.#t("A title is required.");
                this.#messageKind = "alert";
                this.#render();
                return;
            }
            const isNew = item.id === this.#newItem?.id;
            const issues = isNew ? validatePlanning({ ...snapshot, items: [...snapshot.items, next] }, this.#t) : validateItemEdit(snapshot, next, this.#t);
            if (issues.length) {
        ...
        async #createFlow(source, form) {
            ...
            if (!targetId) {
                this.#invalid(this.#t("Choose a sibling to connect."));
                return;
            }
            ...
            const issues = validatePlanning(candidate, this.#t);
            if (issues.length > 0) {
        ...
        async #connectBetween(sourceId, targetId) {
            ...
            const issues = validatePlanning({ ...snapshot, flows: [...snapshot.flows, flow] }, this.#t);
            if (issues.length) {
                this.#invalid(issues[0]);
                return;
            }
        ...
        async #deleteFlow(flow, selectedId) {
            ...
            if (consequences.length && !confirm(this.#t("Remove this flow and its {0} attached consequences?", { "0": consequences.length })))
                return;
        ...
        async #savePositions(positions) { const snapshot = this.#snapshot; if (snapshot === undefined)
            return; await this.#mutate(runtime => runtime.repository.savePositions(snapshot, this.#scopeId, positions), this.#t("Position saved."), this.#selectedId); this.querySelector(".dm-planner-viewport")?.focus({ preventScroll: true }); }
        async #deleteSelection() {
            ...
            if (!snapshot || this.#busy || this.#needsReload || (!items.length && !flows.length))
                return;
            if (!confirm(this.#t("Delete {0} selected items and {1} selected flows, including subtrees and attached annotations? Shared notes keep their other links. Surviving consequences keep their text and lose links to deleted items.", { "0": items.length, "1": flows.length })))
                return;
        ...
        async #delete(item) { const snapshot = this.#snapshot; if (snapshot === undefined || !confirm(this.#t("Delete {0} and its subtree, attached flows and consequences, and incoming planning references? Shared notes will keep their other links. Surviving consequences keep their text and lose links to deleted items.", { "0": item.title })))
            return; await this.#mutate(async (runtime) => { this.#undoDelete = await runtime.repository.deleteSubtree(snapshot, item.id); }, this.#t("Planning subtree deleted.")); }
        async #undoDeletion() {
        ...
        async #resetLayout() {
            if (this.#busy || this.#needsReload || !confirm(this.#t("Reset the positions on this canvas? Items, links and other canvases will stay unchanged.")))
                return;
        ...
        async #addReference(item, form) {
            ...
            try {
                ...
                this.#invalid(plannerError(error, this.#t, this.#t("Planning change failed.")));
                return;
            }
            const reference = { id: `reference-${crypto.randomUUID()}`, schemaVersion: 3, itemId: item.id, name: String(data.get("name") ?? "").trim() || targetLabel(target, snapshot.items, core, this.#t), relation: "related", target, quantity: 1, notes: "", updatedAt: Date.now() };
        ...
        async #addNote(item) { const note = { id: `note-${crypto.randomUUID()}`, schemaVersion: 3, title: this.#t("DM note"), body: "", anchorIds: [item.id], updatedAt: Date.now() }; await this.#mutate(async (runtime, snapshot) => runtime.repository.put(snapshot, "dm_notes", note, 0), this.#t("DM note added."), item.id); }
        async #saveReference(reference, form, selectedId) {
            ...
            if (!Number.isInteger(quantity) || quantity < 1 || quantity > 1000) {
                this.#invalid(this.#t("Quantity must be a whole number from 1 to 1000."));
                return;
            }
            let target;
            try {
            ...
            if (next.name === "") {
                this.#invalid(this.#t("A reference name is required."));
                return;
            }
            await this.#putExisting("planning_references", next, form, selectedId, this.#t("Reference saved."));
        }
        async #saveConsequence(consequence, form, selectedId) {
            ...
            if (anchorKey !== `item:${selectedId}` && !flow) {
                this.#invalid(this.#t("Choose this item or one of its existing flows for the consequence."));
                return;
            }
            ...
            if (next.title === "") {
                this.#invalid(this.#t("A consequence title is required."));
                return;
            }
        ...
        async #saveNote(note, form, selectedId) {
            ...
            if (next.title === "") {
                this.#invalid(this.#t("A DM note title is required."));
                return;
            }
            await this.#putExisting("dm_notes", next, form, selectedId, this.#t("DM note saved."));
        }
        ...
        #removedDrafts(document, root, snapshot) {
            for (const [key, draft] of this.#drafts.entries()) {
                ...
                const discard = actionButton(document, this.#t("Discard removed record edits"), () => { this.#drafts.clear(key); this.#render(); });
                discard.dataset["viewAction"] = "";
        ...
        async #mutate(operation, success, selected, draftKey) {
            ...
            try {
                ...
                if (this.#runtime === runtime && this.isConnected) {
                    this.#needsReload = true;
                    this.#fail(error, this.#t("Planning change failed."));
                }
                return false;
            }
            finally {
        ...
        #fail(error, fallback) { this.#message = plannerError(error, this.#t, fallback); this.#messageKind = "alert"; }
        #unavailable(message) { this.replaceChildren(messageBlock(this.ownerDocument, message, "alert")); const link = this.ownerDocument.createElement("a"); link.textContent = this.#t("Open campaign canvas"); link.href = plannerLink(this.#contribution?.addon.id ?? "dm-tools"); this.append(link); }
    }
    customElements.define(tag, PlannerElement);
    return tag;
}
function appendItemStructure(document, form, item, items, t = plannerTranslator()) {
    const row = document.createElement("div");
    ...
    help.textContent = t("Moving an item keeps its children and annotations. Connected story flows must stay on the same canvas.");
