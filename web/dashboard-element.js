...
export function defineDashboardElement(generation) {
    ...
    class DashboardElement extends HTMLElement {
        ...
        set codexContribution(value) {
            ...
            this.#context = value;
            const locale = dashboardLocale(value.host), changed = locale !== this.#locale;
            this.#locale = locale;
            if (!this.isConnected)
                return;
        ...
        #render() {
            ...
            root.className = "dm-dashboard-stack";
            root.setAttribute("aria-busy", String(this.#status === "loading"));
            const t = (key) => dashboardText(this.#locale, key);
            ...
            const section = (title, level = "h3") => { const node = document.createElement("section"); node.className = "dm-dashboard-section"; node.append(element(level, t(title))); return node; };
            ...
            if (this.#status === "error") {
                const error = element("p", t("dashboard.error.body"), "dm-dashboard-warning");
                error.setAttribute("role", "alert");
                overview.append(element("h3", t("dashboard.error.title")), error);
            }
            const summary = this.#snapshot && summarizePlanning(this.#snapshot, this.#locale);
            if (summary) {
                const totals = document.createElement("div");
                totals.className = "dm-dashboard-totals";
                for (const [key, count] of Object.entries(summary.counts)) {
                    const tile = document.createElement("div");
                    tile.className = `dm-dashboard-tile${key === "total" ? " accent" : ""}`;
                    tile.dataset["stat"] = key;
                    tile.append(element("div", t(`dashboard.status.${key}`), "dm-dashboard-label"), element("div", new Intl.NumberFormat(this.#locale).format(count), "dm-dashboard-value"));
                    totals.append(tile);
                }
                overview.append(totals);
            }
            const refresh = document.createElement("button");
            refresh.className = "dm-dashboard-refresh";
            refresh.textContent = t("dashboard.refresh");
            ...
            const workflows = section("dashboard.workflow.title");
            workflows.append(element("p", t("dashboard.workflow.body"), "dm-dashboard-hint"));
            if (this.#status !== "loading" && this.#provider !== "ready")
                workflows.append(element("p", t(this.#provider === "missing" ? "dashboard.importMissing" : "dashboard.importError"), "dm-dashboard-warning"));
            const nav = document.createElement("nav");
            nav.className = "dm-dashboard-workflows";
            nav.setAttribute("aria-label", t("dashboard.workflow.label"));
            for (const [kind, href] of [["planning", plannerLink(addonId)], ["import", `#/addons/${encodeURIComponent(addonId)}/imports`]]) {
                const link = document.createElement("a");
                link.href = href;
                link.append(element("strong", t(`dashboard.${kind}.title`)), element("span", t(`dashboard.${kind}.body`), "dm-dashboard-hint"));
                nav.append(link);
            }
            workflows.append(nav);
            ...
            if (summary) {
                ...
                if (!summary.recent.length) {
                    ...
                    const link = document.createElement("a");
                    link.className = "dm-dashboard-refresh";
                    link.href = plannerLink(addonId);
                    link.textContent = t("dashboard.empty.action");
                    recent.append(link);
                }
                else {
                    const list = document.createElement("div");
                    list.className = "dm-dashboard-recent";
                    for (const item of summary.recent) {
                        const link = document.createElement("a");
                        link.href = plannerLink(addonId, item.id);
                        link.dataset["recentItem"] = item.id;
                        const text = document.createElement("div");
                        text.append(element("strong", item.title));
                        if (item.summary)
                            text.append(element("div", item.summary, "dm-dashboard-hint"));
                        ...
                        link.append(text, element("span", t(key), "dm-dashboard-badge"));
                        list.append(link);
                    }
                    recent.append(list);
                }
    ...
    customElements.define(tag, DashboardElement);
    return tag;
}
