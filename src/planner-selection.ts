...
export interface CanvasSelection { items: Set<string>; flows: Set<string>; primary: string | undefined }
type Point = { x: number; y: number };
interface CanvasOptions {
  viewport: HTMLElement; stage: HTMLElement; positions: ReadonlyMap<string, Point>; zoom: number;
...
export function mountCanvasSelection(options: CanvasOptions): void {
  ...
  const hull = document.createElement("div"); hull.className = "dm-selection-hull"; hull.hidden = true; stage.append(hull);
  const marquee = document.createElement("div"); marquee.className = "dm-selection-marquee"; marquee.hidden = true; stage.append(marquee);
  const geometry = (card: HTMLElement) => ({ x: parseFloat(card.style.left), y: parseFloat(card.style.top), width: card.offsetWidth || parseFloat(card.style.width), height: card.offsetHeight || parseFloat(card.style.minHeight) });
  const paint = (): void => {
    for (const [id, card] of cards) { card.classList.toggle("selected", selection.items.has(id)); card.setAttribute("aria-pressed", String(selection.items.has(id))); }
    for (const path of stage.querySelectorAll("[data-flow-id]")) path.classList.toggle("selected", selection.flows.has(path.getAttribute("data-flow-id")!));
    for (const hit of stage.querySelectorAll("[data-select-flow]")) hit.setAttribute("aria-pressed", String(selection.flows.has(hit.getAttribute("data-select-flow")!)));
    const boxes = [...selection.items].map(id => cards.get(id)).filter((card): card is HTMLElement => !!card).map(geometry);
  ...
  const change = (items: Set<string>, flows = new Set<string>(), primary = [...items][0]): void => {
    selection = { items, flows, primary }; paint(); options.change(selection);
  };
  const toggle = (id: string, kind: "items" | "flows", additive: boolean): void => {
  ...
  viewport.addEventListener("click", event => {
    ...
    if (card && event.detail === 0) toggle(card.dataset["itemId"]!, "items", event.shiftKey);
  });
  ...
  viewport.addEventListener("keydown", event => {
    ...
    if (modifier && event.key.toLowerCase() === "a") { event.preventDefault(); change(new Set(cards.keys())); }
      ...
      event.preventDefault(); toggle((event.target as Element).getAttribute("data-select-flow")!, "flows", event.shiftKey);
    }
  ...
  viewport.addEventListener("pointerdown", event => {
    ...
    event.preventDefault(); viewport.focus({ preventScroll: true }); suppressClick = false;
    ...
    let collapse = false;
    if (!pan && id) {
      if (event.shiftKey) { toggle(id, "items", true); if (!selection.items.has(id)) return; }
      else if (!selection.items.has(id)) change(new Set([id]));
      else collapse = selection.items.size > 1 || selection.flows.size > 0;
    }
    ...
    const capture = card ?? viewport; capture.setPointerCapture(event.pointerId);
    const move = (next: PointerEvent): void => {
      if (next.pointerId !== event.pointerId) return;
      const x = next.clientX - start.x, y = next.clientY - start.y;
      if (Math.abs(x) + Math.abs(y) > 4) moved = true;
      if (!moved) return;
      if (pan) { viewport.scrollLeft = left - x; viewport.scrollTop = top - y; return; }
      if (selected.length) {
        dx = Math.round(x / zoom / 24) * 24; dy = Math.round(y / zoom / 24) * 24;
        for (const value of selected) { value.card.style.left = `${nativePixel(value.box.x + dx * zoom)}px`; value.card.style.top = `${nativePixel(value.box.y + dy * zoom)}px`; }
        paint(); return;
      }
      ...
      marquee.hidden = false; Object.assign(marquee.style, { left: `${box.x}px`, top: `${box.y}px`, width: `${box.width}px`, height: `${box.height}px` });
      const items = new Set(event.shiftKey ? initial.items : []);
      for (const [key, value] of cards) { const cardBox = geometry(value); if (cardBox.x < box.x + box.width && cardBox.x + cardBox.width > box.x && cardBox.y < box.y + box.height && cardBox.y + cardBox.height > box.y) items.add(key); }
      change(items, new Set(event.shiftKey ? initial.flows : []));
    };
    const end = (next: PointerEvent): void => {
      if (next.pointerId !== event.pointerId) return;
      viewport.removeEventListener("pointermove", move); viewport.removeEventListener("pointerup", end); viewport.removeEventListener("pointercancel", end); viewport.removeEventListener("lostpointercapture", end);
      if (capture.hasPointerCapture(event.pointerId)) capture.releasePointerCapture(event.pointerId);
      marquee.hidden = true; suppressClick = moved;
      if (next.type !== "pointerup") {
        for (const value of selected) { value.card.style.left = `${value.box.x}px`; value.card.style.top = `${value.box.y}px`; }
        change(initial.items, initial.flows, initial.primary); return;
      }
      if (selected.length && moved && (dx || dy)) options.move(Object.fromEntries(selected.map(value => [value.id, { x: value.point.x + dx, y: value.point.y + dy }])));
      else if (!moved && !pan) { if (collapse && id) change(new Set([id])); else if (!id) change(new Set()); }
    };
