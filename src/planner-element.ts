...
export function definePlannerElement(): void {
  ...
  class PlannerElement extends HTMLElement {
    ...
    #render(): void {
      ...
      if (snapshot === undefined) { if (this.#message !== "") this.replaceChildren(messageBlock(this.ownerDocument, this.#message, this.#messageKind)); return; }
    ...
    #flowEditor(document: Document, snapshot: PlanningSnapshot, selected: PlanningItem): HTMLElement {
      ...
      const label = document.createElement("input"); label.placeholder = selected.kind === "branch" ? "Option label" : "Flow label"; section.append(title, select, label, actionButton(document, "Create flow", () => { if (select.value !== "") void this.#createFlow(selected, select.value, label.value); }, "primary"));
    ...
    #makeDraggable(card: HTMLElement, itemId: string, original: { x: number; y: number }): void {
      card.addEventListener("pointerdown", (event) => {
        if (event.button !== 0 || (event.target as Element).closest("button,input,textarea,select") !== null) return;
        const startX = event.clientX; const startY = event.clientY; let moved = false; card.setPointerCapture(event.pointerId);
        const move = (next: PointerEvent): void => { const dx = next.clientX - startX; const dy = next.clientY - startY; if (Math.abs(dx) + Math.abs(dy) > 4) moved = true; card.style.left = `${snap(original.x + dx)}px`; card.style.top = `${snap(original.y + dy)}px`; };
        const end = (next: PointerEvent): void => { card.removeEventListener("pointermove", move); card.removeEventListener("pointerup", end); card.removeEventListener("pointercancel", end); if (moved) void this.#savePosition(itemId, snap(original.x + next.clientX - startX), snap(original.y + next.clientY - startY)); };
    ...
    async #saveItem(item: PlanningItem, form: HTMLFormElement): Promise<void> {
      const data = new FormData(form); const next: PlanningItem = { ...item, title: String(data.get("title") ?? "").trim(), summary: String(data.get("summary") ?? "").trim(), body: String(data.get("body") ?? "").trim(), tags: String(data.get("tags") ?? "").split(",").map((tag) => tag.trim()).filter(Boolean).slice(0, 40), updatedAt: Date.now() };
      if (next.title === "") { this.#message = "A title is required."; this.#messageKind = "alert"; this.#render(); return; }
      const revision = this.#snapshot?.revisions.get(`planning_items:${item.id}`); if (revision === undefined) return;
