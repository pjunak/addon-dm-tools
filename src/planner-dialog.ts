...
export function plannerShortcuts(document: Document, close: () => void, t: PlannerTranslator = plannerTranslator()): HTMLDialogElement {
  const dialog = document.createElement("dialog"); dialog.className = "dm-planner-dialog dm-planner-shortcuts"; dialog.setAttribute("aria-label", t("Keyboard shortcuts"));
  const header = document.createElement("header"), title = document.createElement("h2"), button = document.createElement("button");
  title.textContent = t("Keyboard shortcuts"); button.type = "button"; button.textContent = t("Close shortcuts"); button.dataset["viewAction"] = ""; button.addEventListener("click", close);
