...
async function createStaticServer(root) {
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      const requested = resolve(root, ...decodeURIComponent(url.pathname).split("/").filter(Boolean));
...
async function run({ root, fixture }) {
  const absoluteRoot = resolve(root);
  const absoluteFixture = resolve(absoluteRoot, fixture);
  if (!isContained(absoluteRoot, absoluteFixture)) throw new Error(`Fixture must stay inside the served root: ${fixture}`);
  ...
  try {
    ...
    const fixturePath = relative(absoluteRoot, absoluteFixture).split(sep).map(encodeURIComponent).join("/");
    const url = `http://127.0.0.1:${address.port}/${fixturePath}`;
    const browser = await chromium.launch({ headless: true });
    try {
      for (const deviceScaleFactor of [1, 2]) {
        const context = await browser.newContext({ deviceScaleFactor, locale: "en-US", viewport: { width: 1280, height: 900 } });
        try {
          ...
          for (const check of failedChecks) {
            failures.push(`${prefix} ${check?.name || "unnamed check"}: expected ${JSON.stringify(check?.expected)}, got ${JSON.stringify(check?.actual)}`);
          }
          failures.push(...pageErrors.map(error => `${prefix} ${error}`));
          console.log(`${result.name || fixture} ${prefix}: ${result.checks.length - failedChecks.length}/${result.checks.length} checks passed`);
        } finally {
          await context.close();
        }
      }
  ...
  if (failures.length > 0) throw new Error(`Browser rendering contract failed:\n- ${failures.join("\n- ")}`);
}
